...
2. **TodoList 結構**：
    - 任務應具有「中等粒度」，即一個 Worker 能在一次 ReAct 循環中完成的工作單元。
    - **並行與依序**：透過 `dependencies` 控制。若 `dependencies` 為空，該任務將在系統啟動時立刻並行執行；若需等待，請填入依賴的任務 ID。
    - **指派精準**：根據 `available_agents` 的能力（如 coder, researcher）進行指派。
