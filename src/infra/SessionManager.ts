...
export class SessionManager implements ISessionManager {
  ...
  async createFromJSON(json: Record<string, any>): Promise<BaseSession> {
    ...
    logger.info(`[SessionManager] Creating session from JSON: ${id}`, { type: 'SYSTEM' });
  ...
  async restoreFromSnapshot(snapshot: string): Promise<BaseSession> {
    logger.info(`[SessionManager] Restoring session from snapshot`, { type: 'SYSTEM' });
    try {
      const json = JSON.parse(snapshot);
      return this.createFromJSON(json);
    } catch (error) {
      logger.error(`[SessionManager] Failed to restore session: ${error}`, { type: 'SYSTEM', payload: { error } });
  ...
  deleteSession(id: string): void {
    logger.info(`[SessionManager] Deleting session: ${id}`, { type: 'SYSTEM' });
