...
import { IEvent, IEventBus } from './IBus';
...
export class EventBus<P = any> implements IEventBus<P> {
    ...
    publish<T extends string, PL extends P>(event: IEvent<T, PL>): void {
        ...
        setImmediate(() => {
            LogManager.recorder.debug(`[EventBus] Publishing event: ${event.type} to ${allHandlers.size} subscribers`, { type: 'SYSTEM' });

            allHandlers.forEach(handler => {
                try {
                    handler(event);
                } catch (error) {
                    LogManager.recorder.error(`[EventBus] Subscriber failed for event: ${event.type}`, {
                        type: 'SYSTEM',
    ...
    subscribe<T extends string, PL extends P>(
        ...
        LogManager.recorder.info(`[EventBus] Subscribed to: ${type}`, { type: 'SYSTEM' });
    }
    ...
    unsubscribe<T extends string, PL extends P>(
        ...
        if (handlers) {
            ...
            LogManager.recorder.info(`[EventBus] Unsubscribed from: ${type}`, { type: 'SYSTEM' });
        }
    }
}
